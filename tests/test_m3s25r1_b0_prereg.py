...
def test_b042_negative_a2r_config_id_mismatch():
    """Tamper A2R ledger config_id => CONFIG_ID MISMATCH, sim=0."""
    vc = _get_center_verifier()
    a1r = _load_a1r_inherited()["inherited_units"]
    a2r = _load_a2r_manifest()
    a1r_l = _load_a1r_ledger()
    a2r_l = _load_a2r_ledger()
    # Tamper: change first A2R COMPLETE ledger entry's config_id
    a2r_l_bad = list(a2r_l)
    for i, e in enumerate(a2r_l_bad):
        if e.get("status") == "COMPLETE":
            a2r_l_bad[i] = dict(e)
            a2r_l_bad[i]["config_id"] = "WRONG_CONFIG"
            break
    counter = _SimCounter()
    with patch("hyptraj.m3s25r1.arm_b.draw_online_pilot", counter):
        try:
            ...
            assert "CONFIG_ID MISMATCH" in str(e)
    assert counter.count == 0
